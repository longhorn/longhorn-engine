...
func dumpTable(w io.Writer, path string, records []wal.Record, verbose bool) error {
	...
	if _, err := fmt.Fprintf(w, "Size:    %d bytes, %d records\n\n", st.Size(), len(records)); err != nil {
		return err
	}

	tw := tabwriter.NewWriter(w, 0, 4, 2, ' ', 0)
	if _, err := fmt.Fprintln(tw, "IDX\tTYPE\tTXN\tSUMMARY"); err != nil {
		return err
	}
	for i, r := range records {
		...
		if _, err := fmt.Fprintf(tw, "%d\t%s\t%s\t%s\n", i, r.Type, txnStr, sum); err != nil {
			return err
		}
		if verbose {
			if _, err := fmt.Fprintf(tw, "\t\t\t  payload: %s\n", string(r.Payload)); err != nil {
				return err
			}
		}
	}
...
func printAnalysis(w io.Writer, records []wal.Record, asJSON bool) error {
	...
	if _, err := fmt.Fprintf(w, "  next_txn_id : %d\n", a.NextTxnID); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(w, "  pending     : %d\n", len(a.Pending)); err != nil {
		return err
	}
	for _, pt := range a.Pending {
		...
		if _, err := fmt.Fprintf(w, "    txn %d  op=%s  intents=%d  applied=%d  %s\n",
			pt.ID, pt.Op, len(pt.PendingIntents), len(pt.CompletedSteps), status); err != nil {
			return err
		}
		for _, in := range pt.PendingIntents {
			...
			if _, err := fmt.Fprintf(w, "      [%s] step=%d action=%s\n", done, in.StepID, in.Action); err != nil {
				return err
			}
		}
	}
