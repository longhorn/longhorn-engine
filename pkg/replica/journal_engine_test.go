...
func TestRecoverJournalQuarantinesUnreadable(t *testing.T) {
	dir := t.TempDir()
	// A complete 20-byte header of zeros has bad magic, which is treated
	// as mid-stream corruption and triggers the quarantine path. A shorter
	// write would be read as a torn tail and truncated instead.
	if err := os.WriteFile(filepath.Join(dir, wal.FileName),
		make([]byte, 20), 0600); err != nil {
		t.Fatal(err)
	}
