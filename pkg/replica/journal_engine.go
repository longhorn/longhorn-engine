...
func applyCreateHead(s stepDir, raw []byte) error {
	...
	// CREATE_HEAD is marked STEP_DONE once this function returns, so the head
	// image must be durable before then; otherwise a crash could lose the
	// truncate/creation while recovery skips the step as already done.
	fd, err := os.OpenFile(imgPath, os.O_RDWR, 0)
	if err != nil {
		return errors.Wrapf(err, "open %v for fsync", imgPath)
	}
	if err := fd.Sync(); err != nil {
		_ = fd.Close()
		return errors.Wrapf(err, "fsync %v", imgPath)
	}
	if err := fd.Close(); err != nil {
		return errors.Wrapf(err, "close %v", imgPath)
	}
...
func applyLinkAsSnapshot(s stepDir, raw []byte) error {
	...
	// Both hardlinks are already correct, but a prior attempt may have
	// created them (and left a stale checksum) before crashing ahead of the
	// fsync. Still fsync the directory so the links and the checksum removal
	// above are durable before returning.
	if imgSame && metaSame {
		return lhio.FsyncDir(s.dir)
	}
...
// caller to use for new transactions; close it via wal.Close(). The
// returned count reports how many prepared transactions were rolled
// forward during recovery, so a lazy caller can tell whether the
// on-disk chain advanced past an already-built in-memory view.
func recoverJournal(dir string) (*wal.Journal, int, error) {
	j, qInfo, err := wal.OpenWithQuarantine(dir)
	if err != nil {
		return nil, 0, errors.Wrap(err, "open journal")
	}
	if qInfo != nil {
	...
	if err != nil {
		_ = j.Close()
		return nil, 0, errors.Wrap(err, "recover journal")
	}
	...
	for _, pt := range analysis.Pending {
		if !pt.Prepared {
			...
			if err != nil {
				_ = j.Close()
				return nil, 0, errors.Wrapf(err, "adopt un-prepared txn %d", pt.ID)
			}
			if err := tx.Abort(); err != nil {
				_ = j.Close()
				return nil, 0, errors.Wrapf(err, "abort un-prepared txn %d", pt.ID)
			}
		...
		if err != nil {
			_ = j.Close()
			return nil, 0, errors.Wrapf(err, "adopt prepared txn %d", pt.ID)
		}
		for _, intent := range pt.PendingIntents {
			...
			if !ok {
				...
				_ = j.Close()
				return nil, 0, fmt.Errorf("recovery: unknown action %q in txn %d step %d",
			...
			if err := fn(s, intent.Args); err != nil {
				...
				_ = j.Close()
				return nil, 0, errors.Wrapf(err, "recovery: apply step %d (%s) of txn %d",
					intent.StepID, intent.Action, pt.ID)
			}
			if err := tx.StepDone(intent.StepID); err != nil {
				_ = j.Close()
				return nil, 0, errors.Wrapf(err, "recovery: STEP_DONE for txn %d step %d",
					pt.ID, intent.StepID)
			}
		}
		if err := tx.Commit(); err != nil {
			_ = j.Close()
			return nil, 0, errors.Wrapf(err, "recovery: commit txn %d", pt.ID)
		}
		replayed++
	}
	...
	if err := j.Checkpoint(); err != nil {
		_ = j.Close()
		return nil, 0, errors.Wrap(err, "checkpoint after recovery")
	}
