...
func (t *Task) CreateBackup(backupName, snapshot, dest, backingImageName, backingImageChecksum string,
	compressionMethod string, concurrentLimit int, storageClassName, objectStoreBackup string, labels []string, credential map[string]string) (*BackupCreateInfo, error) {
...
func (t *Task) createBackup(replicaInController *types.ControllerReplicaInfo, backupName, snapshot, dest, volumeName,
	backingImageName, backingImageChecksum, compression string, concurrentLimit int, storageClassName,
	objectStoreBackup string, labels []string, credential map[string]string) (*BackupCreateInfo, error) {
	...
	reply, err := repClient.CreateBackup(backupName, snapshot, dest, volumeName, backingImageName, backingImageChecksum,
		compression, concurrentLimit, storageClassName, objectStoreBackup, labels, credential)
	if err != nil {
		return nil, err
	}
