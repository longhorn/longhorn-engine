...
type Executor struct {
	...
	Utils utils.ExecuteInterface // An interface for executing commands. This allows mocking for unit tests.
}
...
func NewNamespaceExecutor(processName, procDirectory string, namespaces []types.Namespace) (*Executor, error) {
	nsDir, err := utils.GetProcessNamespaceDirectory(processName, procDirectory)
	if err != nil {
		return nil, err
	}
	...
	NamespaceExecutor := &Executor{
		...
		nsDirectory: nsDir,
		Utils:       utils.NewExecutor(),
	}

	if _, err := NamespaceExecutor.Utils.Execute(nil, types.NsBinary, []string{"-V"}, types.ExecuteDefaultTimeout); err != nil {
...
func (nsexec *Executor) Execute(binary string, args []string, timeout time.Duration) (string, error) {
	return nsexec.Utils.Execute(nil, types.NsBinary, nsexec.prepareCommandArgs(binary, args), timeout)
}
...
func (nsexec *Executor) ExecuteWithStdin(binary string, args []string, stdinString string, timeout time.Duration) (string, error) {
	return nsexec.Utils.ExecuteWithStdin(types.NsBinary, nsexec.prepareCommandArgs(binary, args), stdinString, timeout)
}
...
func (nsexec *Executor) ExecuteWithStdinPipe(binary string, args []string, stdinString string, timeout time.Duration) (string, error) {
	return nsexec.Utils.ExecuteWithStdinPipe(types.NsBinary, nsexec.prepareCommandArgs(binary, args), stdinString, timeout)
}
