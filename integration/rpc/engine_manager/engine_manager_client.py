...
class EngineManagerClient(object):
	...
	self.address = url
        ...
        self.stub = rpc_pb2_grpc.LonghornEngineServiceStub(self.channel)

    def engine_create(self, name, volume_name, binary, listen, listen_addr,
                   size, frontend, replicas, backends=["tcp"]):
        if not name or not volume_name or not binary:
            raise Exception("missing parameter")

        if not listen and not listen_addr:
            raise Exception("missing parameter")
    ...
    def engine_upgrade(self, name, binary, size, replicas):
        ...
        return self.stub.EngineCreate(rpc_pb2.EngineUpgradeRequest(
            spec=rpc_pb2.EngineSpec(
                name=name, volume_name="", binary=binary,
                listen="", listen_addr="", size=size,
    ...
    def frontend_shutdown(self, name):
        ...
        return self.stub.FrontendShutdown(rpc_pb2.FrontendShutdownRequest(
            name=name))
    ...
    def frontend_start_callback(self, name):
        ...
        return self.stub.FrontendStartCallback(rpc_pb2.EngineRequest(
            name=name))
    ...
    def frontend_shutdown_callback(self, name):
        ...
        return self.stub.FrontendShutdownCallback(rpc_pb2.EngineRequest(
            name=name))
